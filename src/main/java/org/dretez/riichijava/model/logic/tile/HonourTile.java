...
public final class HonourTile<T extends Enum<T> & Honour<T>> implements Tile, Comparable<HonourTile<?>> {
     ...
     * @see Honour
     */
    public static <T extends Enum<T> & Honour<T>> HonourTile<T> get(T honour) throws IllegalStateException {
    ...
    private static <T extends Enum<T> & Honour<T>> void register(Class<T> type, T[] values) {
